...
function saveLayout() {
	...
	$('body').find(".demo").find("[contenteditable='true']").each(function() {
		var el = $(this);
		if (!el.is('table') &&!el.is('button') && !el.is('a') && !el.is('ul') && !el.is('ol') && !el.is('dl') && !el.is('li') && !el.is('span')) {
			try {
...
$(document).ready(function() {
	...
	$("#edit-mode").click(function(e) {
		...
		$(".sidebar-nav .lyrow").draggable({ disabled:true });
		$(".sidebar-nav .box").draggable({ disabled:true });
		$(this).addClass("active");
		$("#drag-mode").removeClass("active");
		return false;
	});
	$("#drag-mode").click(function(e) {
		e.preventDefault();
		$(".column").sortable({ disabled:false });
		$(".sidebar-nav .lyrow").draggable({ disabled:false });
		$(".sidebar-nav .box").draggable({ disabled:false });
		$(this).addClass("active");
	...
	$("#preview").click(function() {
		...
		var popupStatus = "width=767, height=900, toolbar=no, scrollbars=yes, location=no, status=no, menubar=no, resizable=yes ";
		
		if (layouthistory) {
			window.open("/editor/popup/preview.htm", "uPUSH", popupStatus);
