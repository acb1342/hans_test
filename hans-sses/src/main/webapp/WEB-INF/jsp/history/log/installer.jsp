...
$.datepicker.setDefaults($.datepicker.regional['ko']);
	...
	$(function() {
		...
		$("#usname").click(function() {
			openWindow("/history/log/popup.htm?searchType=101204", "userPopup", "width=448,height=448,top=100,left=100");
		});
	});
...
<form method="get" id="vForm" name="vForm" action="/history/log/installer.htm">
	...
	<fieldset class="searchBox">
		<table align="left" class="searchTbl" style="margin:5px 0 5px 26px" border="0" cellpadding="1" cellspacing="0">
			<tr>
				<!-- 일별 날짜 검색 -->
				<td align="left">
					<input type="radio" name="searchType" value="install" <c:if test="${searchType == 'install'}">checked</c:if>> 설치/고장수리 이력 &nbsp;&nbsp;
				...
				<td align="left">
					<input type="text" id="usname" name="usname" value="${usname}" placeholder="설치자명" readonly/>
	...
	<!-- list _ start -->
	<c:choose>
	<c:when test="${searchType == 'install'}">
		<c:set var="row" value="${ rownum-((page-1)*10) }"/>
		<display:table name="logBuildingOwnerList" id="logBuildingOwner" class="simple" style="margin:5px 0pt;" requestURI="/history/log/installer.htm" pagesize="10" export="false">
			<display:column titleKey="label.history.log.no">
				<c:out value="${row}"/>
				<c:set var="row" value="${row-1}"/>	
			</display:column>
			<display:column titleKey="label.history.log.acceptDate">
				<fmt:parseDate var="strDate" value="${logBuildingOwner.RC_DT}" pattern="yyyy-MM-dd" />
				<fmt:formatDate value="${strDate}" pattern="yyyy-MM-dd" />
			</display:column>
			<display:column titleKey="label.history.log.installerName" property="WK_NAME"/>
			<display:column titleKey="label.history.log.kind">
			<c:choose>
				<c:when test="${logBuildingOwner.WK_TYPE == '802101'}">설치</c:when>
				<c:when test="${logBuildingOwner.WK_TYPE == '802102'}">고장수리</c:when>
			...
			<display:column titleKey="label.history.log.group">
				<c:if test="${logBuildingOwner.WK_TYPE == '802101'}">
				<a href="#" onClick="javascript:popupChargerList('${logBuildingOwner.BD_ID}');">${logBuildingOwner.CHARGER_GROUP_NAME}</a>
				</c:if>
				<c:if test="${logBuildingOwner.WK_TYPE == '802102'}">${logBuildingOwner.CHARGER_GROUP_NAME}</c:if>
			</display:column>
			<display:column titleKey="label.history.log.chargerNumber">
				<c:if test="${logBuildingOwner.WK_TYPE == '802101'}"><a href="#" onClick="javascript:popupChargerList('${logBuildingOwner.BD_ID}');">${logBuildingOwner.MGMT_NO}</a></c:if>
				<c:if test="${logBuildingOwner.WK_TYPE == '802102'}">${logBuildingOwner.MGMT_NO}</c:if>
			</display:column>
			<display:column titleKey="label.history.log.processStatus">
			<c:choose>
			...
			<display:column titleKey="label.history.log.updateDate">
				<fmt:parseDate var="strDate" value="${logBuildingOwner.WK_DT}" pattern="yyyy-MM-dd" />
				<fmt:formatDate value="${strDate}" pattern="yyyy-MM-dd" />
			</display:column>
		</display:table>
	</c:when>
	<c:when test="${searchType == 'pushSend'}">
		...
		<display:table name="logBuildingOwnerList" id="logBuildingOwner" class="simple" style="margin:5px 0pt;" requestURI="/history/log/installer.htm" pagesize="10" export="false">
			...
			<display:column titleKey="label.history.log.logDate" style="width:130px">
				<fmt:formatDate value="${logBuildingOwner.FST_RG_DT}" pattern="yyyy-MM-dd HH:mm:ss"/>
			</display:column>
			<display:column titleKey="label.history.log.userName" property="NAME" style="width:100px"/>
			<display:column titleKey="label.history.log.kind" style="width:150px;">
				<c:choose>
					<c:when test="${!empty logBuildingOwner.CATEGORY}">
						<div style="width:150px; text-align:left; margin-left:40px"><fmt:message key="${logBuildingOwner.CATEGORY}"/></div>
					</c:when>
					<c:otherwise>
						<div style="width:150px; text-align:left; margin-left:40px;">${logBuildingOwner.TITLE}</div>
					</c:otherwise>
				</c:choose>
			</display:column>
			<display:column titleKey="label.history.log.content" style="width:350px;">
				<div style="width:350px; text-align:left; margin-left:30px">${logBuildingOwner.MSG}</div>
			</display:column>
