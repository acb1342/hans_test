...
<script type="text/javascript">
	...
	// 생성 페이지로 이동
	function insert() {
		document.location = "/admin/installer/create.htm";
	}
	...
	function confirmAndDelete() {
		deleteByChecked('/admin/installer/delete.json', function() { 
			search(); 
		});
	}
</script>
<form method="get" id="vForm" name="vForm" action="/admin/installer/search.htm">
	<!-- search _ start -->
	...
	<display:table name="adminList" id="admin" class="simple" style="margin:5px 0pt;" requestURI="/admin/installer/search.htm" pagesize="10" export="false">
		...
		<display:column titleKey="label.cmsUser.installerId" property="id"/>
		<display:column titleKey="label.cmsUser.installerName" property="name"/>
		<display:column titleKey="label.common.authority" property="adminGroup.name"/>
		<display:column titleKey="label.common.account.status">
			<c:choose>
				<c:when test="${admin.validYn == 'Y' }"><fmt:message key="label.common.code.statusY"/></c:when>
				<c:otherwise><fmt:message key="label.common.code.statusN"/></c:otherwise>
			</c:choose>
		</display:column>
		<display:column titleKey="label.common.detail" style="text-align:center; width:140px;" media="html">
			<input type="button" style="width:65px !important;" value='<fmt:message key="label.common.detail"/>' onclick="location.href='/admin/installer/detail.htm?id=${admin.id}'"/>
		<c:if test="${authority.update}">
			<input type="button" style="width:65px !important;" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/admin/installer/update.htm?id=${admin.id}'"/>
		</c:if>
		</display:column>
	</display:table>
	<!-- list _ end -->
