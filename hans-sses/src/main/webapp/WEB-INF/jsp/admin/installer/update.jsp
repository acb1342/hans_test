	...
	$(function() {
		...
		$('#savePwd').click(function(e) {
			...
			if ($("#password").val() == $("#passwordCfm").val()) {
				$.ajax({
					type:'POST',
					url:'/admin/installer/changePassword.json',
					data:{
						id:$("#id").val(),
		...
		$('#save').click(function(e) {
			...
			else if ($('#isNameCheckComplete').val() == '') {
				alert("이름 중복확인을 해주세요.");
				$('#name').focus();
				return;
			}
...
<form method="post" id="vForm" name="vForm" action="/admin/installer/update.htm">
<input type="hidden" id="isNameCheckComplete" value="1"/>
<input type="hidden" id="oldName" value="${admin.name}"/>
<div class="wrap00">
	<!-- input _ start -->
	<table style="width:100%">
		...
		<tr>
			...
			<td colspan="2" class="td02">
				<input type="text" id="name" name="name" onkeydown="javascript:onkeydownName()" value="${admin.name}">
		...
		<tr>
			...
			<td colspan="2" class="td02">
				...
				<input type="button" id="passwordReset" value='<fmt:message key="label.common.resetPwd"/>'/>
				</c:if>
			</td>
		</tr>
		<tr class="line-dot"><td colspan="3"/></tr>
		...
		<tr>
			...
			<c:choose>
				...
				<input type="radio" id="validY" name="validYn" ${admin.validYn == 'Y' ? 'checked' : ''} value="Y"/><fmt:message key="label.common.code.statusY"/>&nbsp;
				<input type="radio" id="validN" name="validYn" ${admin.validYn == 'N' ? 'checked' : ''} value="N"/><fmt:message key="label.common.code.statusN"/>&nbsp;
				</c:when>
				<c:otherwise>
				<input type="radio" id="validY" name="validYn" ${admin.validYn == 'Y' ? 'checked' : ''} value="Y" disabled="disabled"/><fmt:message key="label.common.code.statusY"/>&nbsp;
				<input type="radio" id="validN" name="validYn" ${admin.validYn == 'N' ? 'checked' : ''} value="N" disabled="disabled"/><fmt:message key="label.common.code.statusN"/>&nbsp;
				</c:otherwise>
			</c:choose>
			</td>
		</tr>
