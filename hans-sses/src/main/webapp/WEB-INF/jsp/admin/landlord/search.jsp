...
<script type="text/javascript">
	...
	// 생성 페이지로 이동
	function insert() {
		document.location = "/admin/landlord/create.htm";
	}
	...
	function confirmAndDelete() {
		deleteByChecked('/admin/landlord/delete.json', function() { 
			search(); 
		});
	}
</script>
<form method="get" id="vForm" name="vForm" action="/admin/landlord/search.htm">
	<!-- search _ start -->
	...
	<!-- list _ start -->
	<display:table name="adminList" id="admin" class="simple" style="margin:5px 0pt;" requestURI="/admin/landlord/search.htm" pagesize="10" export="false">
	<c:if test="${authority.delete}">
		...
		<display:column titleKey="label.common.detail" style="text-align:center; width:140px;" media="html">
			<input type="button" style="width:65px !important;" value='<fmt:message key="label.common.detail"/>' onclick="location.href='/admin/landlord/detail.htm?id=${admin.id}'"/>
		<c:if test="${authority.update}">
			<input type="button" style="width:65px !important;" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/admin/landlord/update.htm?id=${admin.id}'"/>
		</c:if>
		</display:column>
	</display:table>
	<!-- list _ end -->
