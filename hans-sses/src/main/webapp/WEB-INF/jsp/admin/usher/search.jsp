...
<script type="text/javascript">
	...
	// 생성 페이지로 이동
	function insert() {
		document.location = "/admin/usher/create.htm";
	}
	...
	function confirmAndDelete() {
		deleteByChecked('/admin/usher/delete.json', function() { 
			search(); 
		});
	}
</script>
<form method="get" id="vForm" name="vForm" action="/admin/usher/search.htm">
	<!-- search _ start -->
	...
	<display:table name="adminList" id="admin" class="simple" style="margin:5px 0pt;" requestURI="/admin/usher/search.htm" pagesize="10" export="false">
		...
		<display:column titleKey="label.cmsUser.helperId" property="id"/>
		<display:column titleKey="label.cmsUser.helperName" property="name"/>
		<display:column titleKey="label.common.authority">${admin.adminGroup.name} 권한</display:column>
		...
		<display:column titleKey="label.common.detail" style="text-align:center; width:140px;" media="html">
			<input type="button" style="width:65px !important;" value='<fmt:message key="label.common.detail"/>' onclick="location.href='/admin/usher/detail.htm?id=${admin.id}'"/>
		<c:if test="${authority.update}">
			<input type="button" style="width:65px !important;" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/admin/usher/update.htm?id=${admin.id}'"/>
		</c:if>
		</display:column>
	</display:table>
	<!-- list _ end -->
