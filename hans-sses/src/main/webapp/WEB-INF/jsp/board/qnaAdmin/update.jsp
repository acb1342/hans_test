...
<script type="text/javascript">
	$(function() {
		...
		$('#questionUpdate').click(function(e) {
			...
			location.href="/board/qnaAdmin/execUpdateQuestion.htm?id=${boadQna.snId}&page=${page}&target=question"+appendUrl;
		});
	...
	function cancel() {
		if(confirm("정말 취소하시겠습니까?")) window.location.href = "/board/qnaAdmin/search.htm";
	}

</script>


<form method="post" id="vForm" name="vForm" action="/board/qnaAdmin/update.htm?id=${boadQna.snId}&page=${page}">
		...
		<tr>
			...
			<td colspan="5" class="td02">
				...
				<c:if test="${loginGroup.id == 2 || loginGroup.id == 3 }">
					<input <c:if test="${'Y' eq boadQna.openYn}">checked</c:if> type="radio" id="openYn" name="openYn" value="Y"> 공개&nbsp;&nbsp;
					<input <c:if test="${'N' eq boadQna.openYn}">checked</c:if> type="radio" id="openYn" name="openYn" value="N"> 비공개
				</c:if>
			</td>
		</tr>
		<tr class="line-dot"><td colspan="6"/></tr>
