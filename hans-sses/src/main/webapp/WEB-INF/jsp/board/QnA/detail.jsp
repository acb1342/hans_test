...
<script type="text/javascript">
	...
	function replyUpdate() {
		...
		location.href = "/board/QnA/replyUpdate.htm?seq=${boadQna.sn_id}";
	}
	...
	// 수정 페이지로 이동
	function update() {
		location.href = "/board/QnA/update.htm?seq=${boadQna.sn_id}&wk=update&page=${page}";
	}
	
	function updateQuestion() {
		location.href = "/board/QnA/updateQuestion.htm?seq=${boadQna.sn_id}&page=${page}";
	}

	// 삭제
	function confirmAndDelete() {
		deleteById('/board/QnA/deleteRepl.json', '${boadQna.sn_id}', function() { 
			search();
		});
	}

	function deleteQuestion() {
		deleteById('/board/QnA/delete.json', '${boadQna.sn_id}', function() { 
			search();
		});
	}
	...
	function search() {
		...
		if (callbackUrl != 'null') {
			location.href = "/board/QnA/search.htm?" + callbackUrl;
		} else {
			location.href = "/board/QnA/search.htm";
		}
	}
...
<input type="hidden" name="page" value="${page}"/>
	...
	<!-- 답변달기 -->
	<form method="post" id="vForm" name="vForm" action="/board/QnA/reply.htm?seq=${boadQna.sn_id}">
	<table style="width:100%">
		<!-- ---------------답변글 있을 때------------------------------------------------------------------ -->
		<c:if test="${'Y' eq replyStatus }">
			...
			<td colspan="2" class="td02">
				<fmt:formatDate value="${replyTime}" pattern="yyyy.MM.dd HH:mm"/>
			</td>
			</tr>
			<tr class="line-dot"><td colspan="6"/></tr>
			<tr>
