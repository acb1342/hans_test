...
<script type="text/javascript">
	...
	// 생성 페이지로 이동
	function insert() {
		document.location = "/board/appVer/create.htm";
	}
</script>
<form method="get" id="vForm" name="vForm" action="/board/appVer/search.htm">
<div class="wrap00">
	<!-- list _ start -->
	<table style="width:100%">
	<c:set var="row" value="${ rownum-((page-1)*10) }"/>
	<display:table name="appVerList" id="appVer" class="simple" style="margin:5px 0pt;" requestURI="/board/appVer/search.htm" pagesize="10" export="false">
		...
		<display:column titleKey="label.common.detail" media="html">
			<input type="button" value='<fmt:message key="label.boad.detail"/>' onclick="location.href='/board/appVer/detail.htm?id=${agent.id}&seq=${appVer.snId}'"/>
			<c:if test="${authority.update}">
				...
				<c:if test="${dateFom1<dateFom2 }" >
					<input type="button" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/board/appVer/update.htm?id=${agent.id}&seq=${appVer.snId}'"/>
				</c:if>
			</c:if>
		</display:column>
	</display:table>
