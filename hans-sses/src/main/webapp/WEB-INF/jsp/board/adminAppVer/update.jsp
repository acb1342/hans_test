...
<form method="post" id="vForm" name="vForm" action="/board/adminAppVer/update.htm" >
<div class="wrap00">
	...
	<table style="width:100%">
					...
					<c:forTokens var="key" items="${targetKeys}" delims="," varStatus="statKey">
						<c:forTokens var="val" items="${targetValues}" delims="," varStatus="statVal">
							<c:if test="${statKey.index == statVal.index}">
								<option <c:if test="${appVer.targetType == key}">selected</c:if> value="${key}">${val}</option>
							</c:if>
						</c:forTokens>
					</c:forTokens>
				</select>
		...
		<tr>
				...
				<select name="OS" id="OS" style="width: 100px">
					<c:forTokens var="key" items="${OSKeys}" delims="," varStatus="statKey">
						<c:forTokens var="val" items="${OSValues}" delims="," varStatus="statVal">
							<c:if test="${statKey.index == statVal.index}">
								<option <c:if test="${appVer.os == key}">selected</c:if> value="${key}">${val}</option>
							</c:if>
						</c:forTokens>
					</c:forTokens>
				</select>
			...
			<td class="td02"><input type="text" name="ver" style="width: 100px" value="${appVer.ver}"/></td>
			<td height="25" class="td01">
				<span class="bul_dot1">◆</span>
				...
				<select name="udt" id="udt" style="width: 100px">
					<c:forTokens var="key" items="${udtKeys}" delims="," varStatus="statKey">
						<c:forTokens var="val" items="${udtValues}" delims="," varStatus="statVal">
							<c:if test="${statKey.index == statVal.index}">
								<option <c:if test="${appVer.updateType == key}">selected</c:if> value="${key}" >${val}</option>
							</c:if>
						</c:forTokens>
					</c:forTokens>
				</select>
		...
		<tr>
			...
			<td colspan="5" class="td02">
				<input id="deployYmd" name="deployYmd" style="width: 100px" value="${appVer.deployYmd}"/>
			<span>
				<select name="hour" style="width: 100px">
					<c:forEach begin="0" end="23" step="1" var="hour">
						<option <c:if test="${beforeHour == hour}">selected</c:if> value="${hour}">${hour}시</option>
					</c:forEach>
				</select>
			</span>
			<span>
				<select name="minute" style="width: 100px">
					<c:forEach begin="0" end="50" step="10" var="minute">
						<option <c:if test="${beforeMinute == minute}">selected</c:if> value="${minute}">${minute}분</option>
					</c:forEach>
				</select>
			</span>
			</td>
		...
		<tr>
			...
			<td colspan="5" class="td02">
				<input type="text" name="url" value="${appVer.url}"/>
			</td>
		</tr>
	</table>
