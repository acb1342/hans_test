...
<script type="text/javascript">
	...
	function cancel() {
		if(confirm("정말 취소하시겠습니까?")) {
			...
			if (callbackUrl != 'null') {
				location.href = "/board/faqAdmin/search.htm?" + callbackUrl;
			} else {
				location.href = "/board/faqAdmin/search.htm";
			}
		}
	}
...
<form method="post" id="vForm" name="vForm" action="/board/faqAdmin/update.htm?id=${boadFaq.snId}">
<div class="wrap00">
	<!-- input _ start -->
	<table style="width:100%">
		...
		<tr>
			...
			<td colspan="2" class="td02">
				<select name="category" id="category">
					<option <c:if test="${'606101' eq boadFaq.category }">selected</c:if> value="0">선택</option>
					<option <c:if test="${'606102' eq boadFaq.category }">selected</c:if> value="1">건물/그룹</option>
					<option <c:if test="${'606103' eq boadFaq.category }">selected</c:if> value="2">충전기정보</option>
					<option <c:if test="${'606104' eq boadFaq.category }">selected</c:if> value="3">설치신청</option>
					<option <c:if test="${'606105' eq boadFaq.category }">selected</c:if> value="4">고장신고</option>
					<option <c:if test="${'606106' eq boadFaq.category }">selected</c:if> value="5">기타</option>
				</select>
			</td>	
		</tr>
		...
		<tr>
			...
			<td colspan="5" class="td02">
				<input type="text" id="question" name="question" value="${boadFaq.question}" />
			</td>
		</tr>
		<tr class="line-dot"><td colspan="6"/></tr>
		<!-- 등록일 -->
		...
		<tr>
			...
			<td colspan="5" class="td02"><textarea maxlength="1200" id="answer" name="answer">${boadFaq.answer}</textarea></td>
		</tr>
		<tr class="line-dot"><td colspan="6"/></tr>
		<!-- status y/n -->
		<tr>
