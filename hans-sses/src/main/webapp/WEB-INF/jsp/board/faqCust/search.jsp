...
<script type="text/javascript">
	...
	// 생성 페이지로 이동
	function insert() {
		document.location = "/board/faqCust/create.htm";
	}
	...
	function confirmAndDelete() {
		deleteByChecked('/board/faqCust/delete.json', function() { 
			search(); 
		});
	}
	
</script>
<form method="get" id="vForm" name="vForm" action="/board/faqCust/search.htm">
	<div class="wrap00">
	<!-- search _ start -->
	...
	<display:table name="boardList" id="boadFaq" class="simple" style="margin:5px 0pt;" requestURI="/board/faqCust/search.htm" pagesize="10" export="false">
		...
		<display:column titleKey="label.common.detail" style="text-align:center; width:100px;" media="html">
			<input type="button" value='<fmt:message key="label.boad.detail"/>' onclick="location.href='/board/faqCust/detail.htm?id=${boadFaq.snId}'"/>
		<c:if test="${authority.update}">
			<input type="button" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/board/faqCust/update.htm?id=${boadFaq.snId}'"/>
		</c:if>
		</display:column>
	</display:table>
	<!-- list _ end -->
