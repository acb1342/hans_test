...
$.datepicker.setDefaults($.datepicker.regional['ko']);
	...
	// 생성 페이지로 이동
	function insert() {
		document.location = "/board/qnaCust/create.htm";
	}
	...
	function confirmAndDelete() {
		deleteByChecked('/board/qnaCust/delete.json', function() { 
			search(); 
		});
	}
</script>
<form method="get" id="vForm" name="vForm" action="/board/qnaCust/search.htm">
	<!-- search _ start -->
	...
	<display:table name="boardList" id="boadQna" class="simple" style="margin:5px 0pt;" requestURI="/board/qnaCust/search.htm" pagesize="10" export="false">
		...
		<display:column titleKey="label.common.detail" style="text-align:center; width:140px;" media="html">
			<c:if test="${'Y' ne boadQna.answerYn}">
				<input type="button" value='<fmt:message key="label.boad.detail"/>' onclick="location.href='/board/qnaCust/update.htm?wk=create&id=${boadQna.snId}&page=${page}'"/>
			</c:if>
			<c:if test="${'Y' eq boadQna.answerYn}">
				<input type="button" value='<fmt:message key="label.boad.detail"/>' onclick="location.href='/board/qnaCust/detail.htm?id=${boadQna.snId}&page=${page}'"/>
			</c:if>
				
			<c:if test="${authority.update}">
				<c:if test="${'Y' eq boadQna.answerYn}">
					<input type="button" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/board/qnaCust/update.htm?wk=update&id=${boadQna.snId}&page=${page}'"/>
				</c:if>
				<c:if test="${'Y' ne boadQna.answerYn}">
	...
	<display:table name="boardList" id="boadQna" class="simple" style="margin:5px 0pt;" requestURI="/board/qnaCust/search.htm" pagesize="10" export="false">
		...
		<c:if test="${loginUser.id ne boadQna.fstRgUsid }">
			<input <c:if test="${'N' eq boadQna.openYn }">disabled</c:if> type="button" value='<fmt:message key="label.boad.detail"/>' onclick="location.href='/board/qnaCust/detail.htm?id=${boadQna.snId}&page=${page}'"/>
		</c:if>
		<c:if test="${authority.update}">
			<c:if test="${'N' eq boadQna.answerYn}">
				<c:if test="${loginUser.id eq boadQna.fstRgUsid }">
					<input type="button" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/board/qnaCust/updateQuestion.htm?id=${boadQna.snId}&page=${page}'"/>
				</c:if>
				<c:if test="${loginUser.id ne boadQna.fstRgUsid }">
					<input type="button" disabled value='<fmt:message key="label.common.modify"/>'/>
				</c:if>
