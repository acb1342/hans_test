...
<script type="text/javascript">
	...
	function replyUpdate() {
		...
		location.href = "/board/qnaCust/replyUpdate.htm?id=${boadQna.snId}";
	}
	...
	// 수정 페이지로 이동
	function update() {
		location.href = "/board/qnaCust/update.htm?id=${boadQna.snId}&wk=update&page=${page}";
	}
	
	function updateQuestion() {
		location.href = "/board/qnaCust/updateQuestion.htm?id=${boadQna.snId}&page=${page}";
	}

	// 삭제
	function confirmAndDelete() {
		deleteById('/board/qnaCust/deleteRepl.json', '${boadQna.snId}', function() { 
			search();
		});
	}

	function deleteQuestion() {
		deleteById('/board/qnaCust/delete.json', '${boadQna.snId}', function() { 
			search();
		});
	}
	...
	function search() {
		...
		if (callbackUrl != 'null') {
			location.href = "/board/qnaCust/search.htm?" + callbackUrl;
		} else {
			location.href = "/board/qnaCust/search.htm";
		}
	}
...
<input type="hidden" name="page" value="${page}"/>
	...
	<!-- 답변달기 -->
	<form method="post" id="vForm" name="vForm" action="/board/qnaCust/reply.htm?id=${boadQna.snId}">
	<table style="width:100%">
		<!-- ---------------답변글 있을 때------------------------------------------------------------------ -->
