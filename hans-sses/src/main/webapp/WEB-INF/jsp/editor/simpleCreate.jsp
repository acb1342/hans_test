...
<script type="text/javascript">
	...
	function selectTemplate() {
		window.open("/editor/popup/sTemplateList.htm","Select a Template", popupStatus);
	}
	...
	function setSelectedTemplate(id) {
		...
		$.getJSON( "/editor/getPushBody.json?editorId=" + id, function(passedData) {
			CKEDITOR.instances.content.setData(passedData.data);
		});
	} 
</script>
</head>
<body class="edit">
	<form method="post" id="vForm" name="vForm" action="/editor/simpleCreate.htm">
		
		<input type="hidden" id="editData" name="editData" value=""/>
		<input type="hidden" id="downloadData" name="downloadData" value=""/>
		<input type="hidden" id="templateFlag" name="templateFlag" value="N"/>
				...
				<table style="width:100%">
					...
					<tr>
						...
						<td colspan="5" class="td02">
							...
							<input class="btn btn-jquery" type="button" value="Load from existing template" onclick="javascript:selectTemplate()"/>
			 		...
			 		<!-- 
			 		 ...
			 		 -->
						<td colspan="6" class="td02">
							<textarea name="content" id="content" style="width:708px; height:420px;">${contentEditor.editData}</textarea>
						</td>
					</tr>
					<tr class="line-dot"><td colspan="6"/></tr>
				</table>
		...
		<div class="" style="float:right">
			<ul class="nav" id="menu-layoutit">
				...
				<c:if test="${authority.create}">
					...
					<button class="btn btn-jquery" id="saveAsTemplate"><fmt:message key="label.common.saveAsTemplate"/></button>
				</c:if>
