...
// 초기화
	...
	// 등록
	function insert() {
		document.location = "/editor/simpleCreate.htm";
	}
					...
					success:function(isCopy) {
						if (isCopy) {
							jAlert('<fmt:message key="statement.copy.success"/>', 'Alert', function() {
								...
								if (template == 'N')
									document.location = "/editor/simpleSearch.htm";
								else
									callback();
							});
						} else {
	...
	function copy(id, template) {
		copyTemplate('/editor/copySimpleTemplate.json', id, template, function() {
			search();
		});
	}
	...
	function confirmAndDelete() {
		deleteByChecked('/editor/simpleDelete.json', function() { 
			search();
		});
	}
...
<form method="get" id="vForm" name="vForm" action="/editor/sTemplate_search.htm">
<div class="wrap00">
	<!-- search _ start -->
	...
	<!-- list _ start -->
	<display:table name="contentEditors" id="contentEditor" class="simple" style="margin:5px 0pt;" requestURI="/editor/sTemplate_search.htm" pagesize="10" export="false">
	<c:if test="${authority.delete}">
		...
		<display:column titleKey="label.common.createDate" media="html" >
			<fmt:formatDate value="${contentEditor.createDate}" pattern="yyyy/MM/dd HH:mm"/>
		</display:column>
		<display:column titleKey="label.common.modifyDate" media="html">
			<fmt:formatDate value="${contentEditor.modifyDate}" pattern="yyyy/MM/dd HH:mm"/>
		</display:column>
		<display:column titleKey="label.common.detail" style="text-align:center; width:250px;" media="html">
		<c:if test="${authority.update}">
			<input type="button" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/editor/sTemplate_update.htm?id=${contentEditor.id}'"/>
			<input type="button" value='<fmt:message key="label.contentEditor.duplicate"/>' onclick="javascript:copy('${contentEditor.id}', 'Y')"/>
		</c:if>
