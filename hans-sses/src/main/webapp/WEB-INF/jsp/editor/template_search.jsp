...
<form method="get" id="vForm" name="vForm" action="/editor/template_search.htm">
<div class="wrap00">
	<!-- search _ start -->
	...
	<!-- list _ start -->
	<display:table name="contentEditors" id="contentEditor" class="simple" style="margin:5px 0pt;" requestURI="/editor/template_search.htm" pagesize="10" export="false">
	<c:if test="${authority.delete}">
		...
		<c:if test="${authority.update}">
			<input type="button" value='<fmt:message key="label.common.modify"/>' onclick="location.href='/editor/template_update.htm?id=${contentEditor.id}'"/>
			<input type="button" value='<fmt:message key="label.contentEditor.templateFlag.Y"/>' onclick="javascript:copy('${contentEditor.id}', 'Y')"/>
			<input type="button" value='<fmt:message key="label.contentEditor.templateFlag.N"/>' onclick="javascript:copy('${contentEditor.id}', 'N')"/>
		</c:if>
