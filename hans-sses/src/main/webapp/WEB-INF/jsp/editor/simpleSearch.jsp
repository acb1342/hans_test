...
<form method="get" id="vForm" name="vForm" action="/editor/simpleSearch.htm">
<div class="wrap00">
	<!-- search _ start -->
	...
	<!-- list _ start -->
	<display:table name="contentEditors" id="contentEditor" class="simple" style="margin:5px 0pt;" requestURI="/editor/simpleSearch.htm" pagesize="10" export="false">
	<c:if test="${authority.delete}">
		...
		<display:column titleKey="label.common.createDate" media="html" style="width:150px;">
			<fmt:formatDate value="${contentEditor.createDate}" pattern="yyyy/MM/dd HH:mm:ss"/>
		</display:column>
		<display:column titleKey="label.common.modifyDate" media="html" style="width:150px;">
			<fmt:formatDate value="${contentEditor.modifyDate}" pattern="yyyy/MM/dd HH:mm:ss"/>
		</display:column>
		<display:column titleKey="label.common.detail" style="text-align:center; width:255px;" media="html">
			<input type="button" value='<fmt:message key="label.common.detail"/>' onclick="location.href='/editor/simpleDetail.htm?id=${contentEditor.id}'"/>
