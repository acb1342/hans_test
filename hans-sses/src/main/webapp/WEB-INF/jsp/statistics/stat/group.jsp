...
<c:set var="group" value='<%=request.getParameter("group")%>'/>
...
$.datepicker.setDefaults($.datepicker.regional['ko']);
	...
	$(function() {
		...
		$("#searchBtn").click(function() {
			...
			if (buildingDetail < 0) { 
				alert("조회하고자 하는 상세/동명을 먼저 선택 후 조회해 주세요.");
				return;
			}
			
			var group = $('select[name="group"]').val();
			if (group < 0) { 
				alert("조회하고자 하는 충전그룹명을 먼저 선택 후 조회해 주세요.");
				return;
			}
	...
	function saveSelected() {
		...
		document.getElementById("chargerGroupSelect").value = targetGroup.options[targetGroup.selectedIndex].value;
	}
...
<form method="get" id="vForm" name="vForm" action="/statistics/stat/group.htm">
	<div class="wrap00">
	<!-- search _ start -->
		...
		<table align="left" class="searchTbl" style="margin:5px 0 5px 26px" border="0" cellpadding="1" cellspacing="0">
			<tr>
				...
				<td align="left">
					...
					<input type="text" id="bdGroup" name="bdGroup" value="${bdGroup}" style="width: 150px;" placeholder="<fmt:message key="label.elcg.buildingName"/>" readonly/>
	...
	<c:when test="${searchType == 'daily'}">
		...
		<display:table name="groupStat" id="groupStat" class="simple tbl_type2" style="margin:5px 0pt;" requestURI="/statistics/stat/group.htm" pagesize="10" export="false">
			<display:column titleKey="label.statistics.stat.no">
				<c:out value="${row}"/>
				<c:set var="row" value="${row-1}"/>	
			</display:column>
			<display:column titleKey="label.statistics.stat.statDate">
				<fmt:parseDate var="strDate" value="${groupStat.CHARGE_YMD}" pattern="yyyyMMdd" />
			...
			<display:column titleKey="label.statistics.stat.group" property="CHARGER_GROUP_NAME"/>
			...
			<display:column titleKey="label.statistics.stat.chargeCount" style="text-align: right; padding-right:10px;">
				<fmt:formatNumber value="${groupStat.CHARGE_CNT}" pattern="#,###"/>
			</display:column>
			<display:column titleKey="label.statistics.stat.totalAmt" style="text-align: right; padding-right:10px;">
				<fmt:formatNumber value="${groupStat.CHARGE_AMT/1000}" pattern="#,###.#"/>
			</display:column>
		</display:table>
	</c:when>
	<c:when test="${searchType == 'monthly'}">
		<c:set var="row" value="${ rownum-((page-1)*10) }"/>
		<display:table name="groupStat" id="groupStat" class="simple" style="margin:5px 0pt;" requestURI="/statistics/stat/group.htm" pagesize="10" export="false">
			<display:column titleKey="label.statistics.stat.no">
				<c:out value="${row}"/>
				<c:set var="row" value="${row-1}"/>	
			</display:column>
			<display:column titleKey="label.statistics.stat.statDate">
				<fmt:parseDate var="strDate" value="${groupStat.CHARGE_YM}" pattern="yyyyMM" />
			...
			<display:column titleKey="label.statistics.stat.group" property="CHARGER_GROUP_NAME"/>
			<display:column titleKey="label.statistics.stat.totalChargeTime" property="DIFF"/>
			<display:column titleKey="label.statistics.stat.chargeCount" style="text-align: right;">
				<fmt:formatNumber value="${groupStat.CHARGE_CNT}" pattern="#,###"/>
			</display:column>
			<display:column titleKey="label.statistics.stat.totalAmt" style="text-align: right;">
				<fmt:formatNumber value="${groupStat.CHARGE_AMT/1000}" pattern="#,###.#"/>
			</display:column>
		</display:table>
	</c:when>
