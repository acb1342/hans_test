...
<form method="get" id="vForm" name="vForm" action="/statistics/stat/buildingPopup.htm" >

	<div class="wrap_winpop">
		<div class="header_area">
			<h1 class="tit"><fmt:message key="label.statistics.stat.buildingPopup"/></h1>
		</div>
		...
		<div class="content_area">
			...
			<c:if test="${searchValue != null && searchValue != ''}">
				<display:table name="buildingList" id="building" class="simple" style="margin:5px 0pt;" requestURI="/statistics/stat/buildingPopup.htm" >
					<display:column titleKey="label.statistics.stat.buildingName" media="html" >
						<a href='javascript:sendChildValue("${building.chargerName}", "${building.chargerName}")'>${building.chargerName}</a>
					</display:column>
					<display:column titleKey="label.statistics.stat.buildingAddress" property="chargerName"/>
				</display:table>
			</c:if>
		</div>
	</div>
