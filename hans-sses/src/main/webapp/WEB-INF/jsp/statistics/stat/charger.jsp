	...
	$(function() {
		...
		$("#searchBtn").click(function() {
			...
			if (!searchType) { 
 			...
 			var chargerNumber = $('input:text[name="chargerNumber"]').val();
			if (chargerNumber == "") { 
				alert("충전기 관리번호를 입력하신 후 조회해 주세요.");
				return;
			}
...
<form method="get" id="vForm" name="vForm" action="/statistics/stat/charger.htm">
	<div class="wrap00">
	<!-- search _ start -->
	...
	<c:when test="${searchType == 'daily'}">
		...
		<display:table name="chargerStat" id="chargerStat" class="simple" style="margin:5px 0pt;" requestURI="/statistics/stat/charger.htm" pagesize="10" export="false">
			<display:column titleKey="label.statistics.stat.no">
				<c:out value="${row}"/>
				<c:set var="row" value="${row-1}"/>	
			</display:column>
			<display:column titleKey="label.statistics.stat.daily">
				<fmt:parseDate var="strTime" value="${chargerStat.START_DT}" pattern="yyyy-MM-dd"/>
				<fmt:formatDate value="${strTime}" pattern="yyyy-MM-dd" />
			</display:column>
			<display:column titleKey="label.statistics.stat.chargerNumber" property="MGMT_NO"/>
			<display:column titleKey="label.statistics.stat.authTime">
				<fmt:parseDate var="strTime" value="${chargerStat.START_DT}" pattern="yyyy-MM-dd HH:mm:ss.S"/>
				<fmt:formatDate value="${strTime}" pattern="HH:mm:ss" />
			</display:column>
			<display:column titleKey="label.statistics.stat.authMethod">
			<c:choose>
				<c:when test="${chargerStat.CERT_TYPE == '315101'}">NFC</c:when>
				<c:when test="${chargerStat.CERT_TYPE == '315102'}">RFID</c:when>
				<c:otherwise>-</c:otherwise>
			</c:choose>
			</display:column>
			<display:column titleKey="label.statistics.stat.authInfo">
				<c:set var="length" value="${fn:length(chargerStat.CERT_CARD_NO)}"/>
				<c:if test="${length > 4}">
				${fn:substring(chargerStat.CERT_CARD_NO, length-4, length)}
				</c:if>
			</display:column>
			<display:column titleKey="label.statistics.stat.startTime">
				<fmt:parseDate var="strTime" value="${chargerStat.START_DT}" pattern="yyyy-MM-dd HH:mm:ss.S"/>
				<fmt:formatDate value="${strTime}" pattern="HH:mm:ss" />
			</display:column>
			<display:column titleKey="label.statistics.stat.endTime">
				<fmt:parseDate var="strTime" value="${chargerStat.END_DT}" pattern="yyyy-MM-dd HH:mm:ss.S"/>
				<fmt:formatDate value="${strTime}" pattern="HH:mm:ss" />
			</display:column>
			<display:column titleKey="label.statistics.stat.useTime" property="DIFF"/>
			<display:column titleKey="label.statistics.stat.amt">
				<fmt:formatNumber value="${chargerStat.CHARGE_AMT/1000}" pattern="#,###.#"/>
			</display:column>
		</display:table>
	</c:when>
	<c:when test="${searchType == 'monthly'}">
		...
		<display:table name="chargerStat" id="chargerStat" class="simple" style="margin:5px 0pt;" requestURI="/statistics/stat/charger.htm" pagesize="10" export="false">
			...
			<display:column titleKey="label.statistics.stat.monthly" property="CHARGE_YM"/>
			<display:column titleKey="label.statistics.stat.chargerNumber" property="MGMT_NO"/>
			<display:column titleKey="label.customer.charge.count" property="CHARGE_CNT"/>
			<display:column titleKey="label.statistics.stat.useTime" property="DIFF"/>
			<display:column titleKey="label.statistics.stat.amt">
				<fmt:formatNumber value="${chargerStat.CHARGE_AMT/1000}" pattern="#,###.#"/>
			</display:column>
			<display:column title="충전요금">
				<fmt:formatNumber value="${chargerStat.CHARGE_FEE}" pattern="#,###.#"/>
			</display:column>
		</display:table>
	</c:when>
	<c:otherwise></c:otherwise>
