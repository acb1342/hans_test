...
<script type="text/javascript">
	...
	function getApps(os) {
		var cpid = $("#cpSn option:selected").val();
        $.ajax({
            ...
            success: function(appsList) {
                var $appSel = $("#appId");
                $appSel.html("");
                $appSel.append('<option value="">' + '<fmt:message key="label.common.option"/>' + '</option>');
                for(var i = 0; i < appsList.length; i++) {
                    var apps = appsList[i];
	                $appSel.append('<option value='+apps.id+'>'+apps.appName+'</option>');
                }
            }
        });
    }
...
<form method="post" id="vForm" name="vForm" action="/push/create.htm">
<div class="wrap00">
	<!-- input _ start -->
	<table style="width:100%">
		...
		<tr>
			...
			<c:if test="${loginGroup.name eq 'ADMIN'}">
				<td colspan="6" class="td02">
					<select id="cpSn" name="cpSn">
					<option value=""><fmt:message key="label.common.option"/></option>
					<c:forEach items="${contentProviders}" var="cps">
						<option value="${cps.id}">${cps.cpName}</option>
					</c:forEach>
					</select>
				</td>
			</c:if>
			<c:if test="${loginGroup.name != 'ADMIN'}">
				<td colspan="6" class="td02">
					<input type="hidden" id="cpSn" name="cpSn" value="${contentProviders.id}"">
					${contentProviders.cpName}
				</td>
			</c:if>			
		</tr>
		...
		<tr class="hidableTr3">
			...
			<td colspan="6" class="td02">
				<input type="text" name="reservedSendTime" id="reservedSendTime" value="<fmt:formatDate value="${reservedSendTime}" pattern="yyyy/MM/dd" />" >
		...
		<tr>
			...
			<td colspan="6" class="td02">
				<input type="radio" id="sendMsgType" name="sendMsgType" value="BOTH" style="width:20px;" checked="checked">Both (Noti+Push)
				<input type="radio" id="sendMsgType" name="sendMsgType" value="NOTI" style="width:20px;">Noti Only
				<input type="radio" id="sendMsgType" name="sendMsgType" value="PUSH" style="width:20px;">Push Only
			</td>
		</tr>
		<tr class="line-dot"><td colspan="6"/></tr>		
		<!-- Noti Title -->
		...
		<tr class="hidableTr5">
			...
			<td colspan="6" class="td02">
				...
				<input type="button" onclick="selectPushBody()" value="<fmt:message key='label.common.select'/>" style="width: 80px;">			
			</td>
		</tr>
		<tr class="line-dot hidableTr5"><td colspan="6"/></tr>
		<!-- Push Button -->
