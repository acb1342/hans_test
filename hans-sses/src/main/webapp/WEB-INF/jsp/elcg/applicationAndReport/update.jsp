...
<script type="text/javascript">
	$(function() {
		...
		if( $('#wkType').val() == '802101') $('#trReport').hide();
	...
	function setChildValue(name, id) {
		...
		$('#bdSelect').append("<option value=''>상세/동명 선택</option>");
		$('#tdOwnerName').empty();
		$('#tdOwnerPhone').empty();
		$('#tdLatitude').empty();
		$('#tdAddr').empty();
	...
	function setChargerGroupSelect() {
		...
		$('#chargerGroupSelect').append("<option>충전그룹 선택</option>");
		$.ajax({
			type:'POST',
	...
	function validationCheck() {
		...
		var bdgId = $('#bdGroupId').val();
		var bdId = $('#bdSelect').val();
		var mgmtNo = $('#setChargerMgmtNoSelect').val();
		var wkType = $('#wkType').val();
		if (bdgId != '') {
			if( $('#bdSelect option').index($('#bdSelect option:selected')) == '' && bdId == '') {
				alert("건물정보를 불러온후에 상세/동명을 선택하세요!");
				return;
			}
			if(wkType == '802102' && $('#setChargerMgmtNoSelect option').index($('#setChargerMgmtNoSelect option:selected')) == '' && mgmtNo == '') {
				alert("충전그룹/관리번호를 선택하세요.");
				return;
			}
		}
...
<form method="post" id="vForm" name="vForm" action="/elcg/applicationAndReport/update.htm" >
<div class="wrap00">
	<!-- input _ first -->
	<input type="hidden" name="wkType" id="wkType" value="${wkType}" />
	<input type="hidden" name="status" value="${obj.status}" />
	<input type="hidden" name="seq" value="${obj.snId}" />
	<input type="hidden" id="bdGroupId" name="bdGroupId" value="${obj.bdGroupId}" />
	...
	<table style="width:100%">
		...
		<tr>
			...
			<td colspan="5" class="td02">
				<textarea id="content" name="content">${obj.body}</textarea>
			</td>
		</tr>
		<tr class="line-dot"><td colspan="6"/></tr>
	</table>
	...
	<c:if test="${loginGroup.id == 1 }">
		<table style="width:100%; margin:10px 0px 5px 0px">
			...
			<tr>
				...
				<c:set var="bdGroup" value="${obj.bdGroupId}"/>
				<td style="width:30%" class="td02" id="tdBdGroup"><c:out value="${obj.bd.bdGroup.name}"/></td>
				<td height="25" class="td01">
					<span class="bul_dot1">◆</span>
					<fmt:message key="label.elcg.ownerName"/>
				</td>
				<td colspan="2" class="td02" id="tdOwnerName">${owner.name}</td>
			</tr>
			<tr class="line-dot"><td colspan="6"/></tr>
			<tr>
				<td style="width:20%" height="25" class="td01">
				...
				<c:set var="bdSelect" value="${obj.bd.bdId}"/>
				<td style="width:30%" class="td02">
					<select id="bdSelect" name="bdSelect" onchange="javascript:loadBdInfo()">
						<c:choose>
							...
							<c:otherwise>
								<option value="${bdSelect}">${obj.bd.name}</option>
							</c:otherwise>
						</c:choose>
					</select>
				</td>
			...
			<tr>
				...
				<td style="width:30%" class="td02" id="tdAddr">${obj.bd.bdGroup.addr}</td>
				<td height="25" class="td01">
					<span class="bul_dot1">◆</span>
			...
			<tr id="trReport">
				...
				<td colspan="5" class="td02">
						<c:set var="chargerGroupSelect" value="${charger.chargerGroup.chargerGroupId}" />
						<select id="chargerGroupSelect" name="chargerGroupSelect" onchange="javascript:setMgmtNoSelect()">
							<c:choose>
								<c:when test="${bdSelect eq '' || bdSelect eq null}">
									<option>충전그룹 선택</option>
								</c:when>
								<c:otherwise>
									<option value="${chargerGroupSelect}">${charger.chargerGroup.name}</option>
								</c:otherwise>
							</c:choose>
						</select>
						
						<c:set var="setChargerMgmtNoSelect" value="${charger.chargerId}"/>
						<select id="setChargerMgmtNoSelect" name="setChargerMgmtNoSelect">
							<c:choose>
								...
								<c:otherwise>
									<option value="${setChargerMgmtNoSelect}">${charger.chargerList.mgmtNo}</option>
								</c:otherwise>
							</c:choose>
						</select>
				</td>
	...
	<!-- input _ third -->
	<c:if test="${loginGroup.id == 1 }">
	<input type="hidden" id="installerId" name="installerId" value="${installer.id}"/>
	<table style="width:100%; margin:10px 0px 5px 0px">
		<tr class="line-top"><td colspan="6"/></tr>
		<tr>
			<td style="width:20%" height="25" class="td01">
			...
			<td style="width:30%" class="td02" id="tdInstaller">
				<c:if test="${obj.status ne '407101' && obj.status ne '409101'}">${installer.name}</c:if>
			</td>
			<td height="25" class="td01">
				<span class="bul_dot1">◆</span>
				<fmt:message key="label.elcg.installerPhone"/>
			</td>
			<td colspan="2" class="td02" id="tdInstallerPhone">
				<c:if test="${obj.status ne '407101' && obj.status ne '409101'}">${installer.mobile}</c:if>
			</td>
		</tr>
		<tr class="line-dot"><td colspan="6"/></tr>
	</table>
