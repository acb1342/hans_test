...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#bdGroupName').autocomplete({
			minLength:2,
			source : function(request, response) {
				$.ajax({
					type: 'post',
					url: '/elcg/controller/searchPopup.json',
					dataType: "json",
					data : {
						search : request.term
					},
...
<form method="get" id="vForm" name="vForm" action="/elcg/controller/popup.htm" >
	...
	<div class="wrap_winpop">
		...
		<display:table name="list" id="list" class="simple" style="margin:5px 0pt;" requestURI="/elcg/controller/popup.htm" >
			<display:column titleKey="label.elcg.buildingName" media="html" >
				<a href='javascript:sendChildValue("${list.name}","${list.bdGroupId}")'>${list.name}</a>
			</display:column>
