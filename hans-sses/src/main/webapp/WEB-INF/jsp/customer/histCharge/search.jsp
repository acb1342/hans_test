...
<form method="get" id="vForm" name="vForm" action="/customer/histCharge/search.htm">
	<div class="wrap00">
	<!-- search _ start -->
	...
	<c:when test="${searchType == 'daily'}">
		...
		<display:table name="histChargeList" id="histCharge" class="simple" style="margin:5px 0pt;" requestURI="/customer/histCharge/search.htm" pagesize="10" export="false">
			<display:column titleKey="label.customer.no">
				<c:out value="${row}"/>
				<c:set var="row" value="${row-1}"/>	
			</display:column>
			<display:column titleKey="label.statistics.stat.statDate">
				<fmt:parseDate var="strDate" value="${histCharge.START_DT}" pattern="yyyy-MM-dd" />
			...
			<display:column titleKey="label.customer.charge.startTime">
				<fmt:parseDate var="strDate" value="${histCharge.START_DT}" pattern="yyyy-MM-dd HH:mm:ss.S" />
				<fmt:formatDate value="${strDate}" pattern="HH:mm:ss" />
			</display:column>
			<display:column titleKey="label.customer.charge.endTime">
				<fmt:parseDate var="strDate" value="${histCharge.END_DT}" pattern="yyyy-MM-dd HH:mm:ss.S" />
				<fmt:formatDate value="${strDate}" pattern="HH:mm:ss" />
			</display:column>
			<display:column titleKey="label.customer.charge.time">
				<fmt:parseDate var="strDate" value="${histCharge.DIFF}" pattern="HH:mm:ss" />
				<fmt:formatDate value="${strDate}" pattern="HH:mm:ss" />
			</display:column>
			<display:column titleKey="label.customer.certType">
				<c:choose>
				<c:when test="${histCharge.CERT_TYPE == '315101'}">안드로이드</c:when>
				<c:when test="${histCharge.CERT_TYPE == '315102'}">
			...
			<display:column titleKey="label.customer.charge.amt">
				<fmt:formatNumber value="${histCharge.CHARGE_AMT/1000}" pattern="#,###.#"/>
			</display:column>
			<display:column titleKey="label.customer.charge.fee">
				<fmt:formatNumber value="${histCharge.CHARGE_FEE}" pattern="#,###"/>
			</display:column>
		</display:table>
	</c:when>
	<c:when test="${searchType == 'monthly'}">
		...
		<display:table name="histChargeList" id="histCharge" class="simple" style="margin:5px 0pt;" requestURI="/customer/histCharge/search.htm" pagesize="10" export="false">
			...
			<display:column titleKey="label.customer.byMonth">
				<fmt:parseDate var="strDate" value="${histCharge.START_DT}" pattern="yyyyMM" />
				<a href="#" onclick="sendToDaily('${histCharge.START_DT}');"><fmt:formatDate value="${strDate}" pattern="yyyy-MM" /></a>
			</display:column>
			<display:column titleKey="label.customer.user.name" property="CUST_NAME"/>
			<display:column titleKey="label.customer.charge.totalTime" property="DIFF">
			</display:column>
			<display:column titleKey="label.customer.charge.count" property="CHARGE_CNT"/>
			<display:column titleKey="label.customer.charge.totalAmt">
				<fmt:formatNumber value="${histCharge.CHARGE_AMT/1000}" pattern="#,###.#"/>
			</display:column>
			<display:column titleKey="label.customer.charge.totalFee">
				<fmt:formatNumber value="${histCharge.CHARGE_FEE}" pattern="#,###"/>
			</display:column>
		</display:table>
	</c:when>
